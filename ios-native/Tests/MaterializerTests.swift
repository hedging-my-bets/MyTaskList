...
final class MaterializerTests: XCTestCase {
    func testSeriesMaterialization() {
        ...
        let st = State(schemaVersion: 2, dayKey: monday, tasks: [], pet: PetState(stageIndex: 0, stageXP: 0, lastCloseoutDayKey: monday), series: [tseries], overrides: [], completions: [:], rolloverEnabled: false, graceMinutes: 60, resetTime: nil)
    ...
    func testOverrideDelete() {
        ...
        let st = State(schemaVersion: 2, dayKey: monday, tasks: [], pet: PetState(stageIndex: 0, stageXP: 0, lastCloseoutDayKey: monday), series: [s], overrides: [ov], completions: [:], rolloverEnabled: false, graceMinutes: 60, resetTime: nil)
        let mats = materializeTasks(for: monday, in: st)
    ...
    func testOneOffOnlySunday() {
        ...
        let st = State(schemaVersion: 2, dayKey: sunday, tasks: [one], pet: PetState(stageIndex: 0, stageXP: 0, lastCloseoutDayKey: sunday), series: [], overrides: [], completions: [:], rolloverEnabled: false, graceMinutes: 60, resetTime: nil)
