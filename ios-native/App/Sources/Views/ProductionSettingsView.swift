...
struct GraceMinutesControl: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                Label("Grace Period", systemImage: "clock.badge.checkmark")
                    .font(.headline)
                Spacer()
                Text("\(store.state.graceMinutes) min")
                    .font(.headline)
                    .foregroundColor(.secondary)
            }

            Text("How close to the scheduled hour a task counts as 'now'. Tasks within this window appear on your Lock Screen widget.")
                .font(.caption)
                .foregroundColor(.secondary)
            ...
            Slider(
                value: Binding(
                    get: { Double(store.state.graceMinutes) },
                    set: { newValue in
                        let minutes = Int(newValue)
                        store.updateGraceMinutes(minutes)

                        // Provide haptic feedback for significant changes
                        if abs(minutes - store.state.graceMinutes) >= 5 {
            ...
            HStack(spacing: 8) {
                ForEach([15, 30, 45, 60], id: \.self) { preset in
                    Button("\(preset)m") {
                        store.updateGraceMinutes(preset)
